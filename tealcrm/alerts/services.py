...
from team.plan_limits import can_use_email_reminders
...
def _build_email_body(request, session):
    ...
    if request:
        ...
        session_url = f'{site_url}{session_path}'
...
def send_upcoming_session_email_reminders(request, user, team):
    if not user or not team or not user.email or not can_use_email_reminders(team):
        return 0
